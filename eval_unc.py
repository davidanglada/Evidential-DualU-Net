...
#
# Test/evaluate DualUNet with Evidential heads (seg + centroid) on your dataset.
# Uses evaluate_test from the DualUNet engine (which now handles:
#   - seg EDL uncertainties (ale, epi, vacuity, entropy)
#   - cent uncertainties (mass, peak, shift, all)
#   - raw and 2D-filtered detection / per-class metrics).
...
from dual_unet.utils.distributed import (
...
# ⬇️ engine.evaluate_test is the one we just updated (with MultiTaskEvaluationMetric)
from dual_unet.engine import evaluate_test_unc
...
def test(cfg):
    ...
    # 2) Optional wandb
    exp = cfg["experiment"]
    exp["wandb"] = exp.get("wandb", False)
    if exp["wandb"] and is_main_process():
        wandb.init(
            project=exp["project"],
            name=exp["name"],
            config=cfg,
            group=exp.get("wandb_group", None),
        )

    # 3) Reproducibility
    seed = exp["seed"] + get_rank()
    ...
    criterion = nn.Identity().to(device)  # evaluate_test ignores it
    ...
    # 8) Evaluate (engine.evaluate_test handles seg+cent metrics + filtering)
    eval_cfg = cfg.get("evaluation", {})
    thresholds = eval_cfg.get("thresholds", [0.5])              # (still there if you want buckets)
    max_pair_distance = eval_cfg.get("max_pair_distance", 12.0) # pairing radius (pixels)
    th_centroid = eval_cfg.get("centroid_h", 0.15)              # h-maxima threshold

    test_stats = evaluate_test_unc(
        cfg=cfg,
        model=model,
        criterion=criterion,            # kept for signature; not used
        data_loader=test_loader,
        device=device,
        thresholds=thresholds,
        max_pair_distance=max_pair_distance,
        output_sufix=exp["name"],
        train=False,
        th=th_centroid,
    )

    # The engine returns {"f": {...}}; flatten with "test_" prefix
    if (
        isinstance(test_stats, dict)
        and "f" in test_stats
        and isinstance(test_stats["f"], dict)
    ):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Test/evaluate DualUNet (EDL seg + centroid)."
    )
    parser.add_argument(
